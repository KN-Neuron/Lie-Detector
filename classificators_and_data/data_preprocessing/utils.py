...
def preprocess_eeg_data(
    ...
    for block_path in blocks_paths:
        ...
        preprocessed_data.append((raw, events, events_timestamps, event_dict))
        all_events_durations.append(events_timestamps)

    return preprocessed_data, all_events_durations
...
def process_fixed_time_epochs(
    ...
    for key, (raw, events, events_timestamps, event_dict) in enumerate(preprocessed_data, start=1):
        undesired_answer, personality = _determine_conditions(key)
        events, events_timestamps = _discard_undesired_responses(events, event_dict, undesired_answer, events_timestamps)
...
def process_variable_time_epochs(
    ...
    for key, (raw, events, events_timestamps, event_dict) in enumerate(preprocessed_data, start=1):
        ...
        label = _determine_honesty(key)
        events, event_id = _rename_events(events, event_id, label)
        epochs, epoches_durations = _create_variable_time_epochs(raw, events, event_id, events_timestamps, baseline=baseline)
        epochs_dict[key] = epochs
        all_epoches_durations_dict[key] = epoches_durations


    padded_array_epochs, labels = _pad_epochs(constant_values, [epoch for sublist in epochs_dict.values() for epoch in sublist])
    
    return epochs_dict, padded_array_epochs, labels, all_epoches_durations_dict
...
def _get_block_paths(dir_path: str) -> list[str]:
    ...
    """
    first_block_path = os.path.join(dir_path, "EEG_ExperimentBlock.HONEST_RESPONSE_TO_TRUE_IDENTITY_raw.fif")
    second_block_path = os.path.join(dir_path, "EEG_ExperimentBlock.DECEITFUL_RESPONSE_TO_TRUE_IDENTITY_raw.fif")
    third_block_path = os.path.join(dir_path, "EEG_ExperimentBlock.DECEITFUL_RESPONSE_TO_FAKE_IDENTITY_raw.fif")
    fourth_block_path = os.path.join(dir_path, "EEG_ExperimentBlock.HONEST_RESPONSE_TO_FAKE_IDENTITY_raw.fif")
...
def _compute_event_durations(raw: mne.io.Raw) -> np.ndarray:
    ...
    end_event = np.append(end_event, 1) # Add a helper value at the end to enable calculation of the duration for the last event
    events_duration = end_event - annotations_time # annotations_time represents start_time
...
def _discard_undesired_responses(
    events: np.ndarray, event_dict: dict[str, int], undesired_answer: str, events_timestamp: np.ndarray
    ...
    undesired_responses = ['ParticipantResponse.TIMEOUT', f'ParticipantResponse.{undesired_answer}']
    indices_to_remove = []
    ...
    filtered_events = events[~np.isin(np.arange(events.shape[0]), unique_indices_to_remove)]
    filtered_events_timestamp = events_timestamp[~np.isin(np.arange(events_timestamp.shape[0]), unique_indices_to_remove)]
...
def _pad_epochs(
    ...
    for epochs in epochs_list:
        ...
        if current_length < max_length:
            ...
            padded_epoch_array = np.pad(epochs.get_data(), pad_width, mode='constant', constant_values=constant_values)
...
def _create_variable_time_epochs(
    raw_file: mne.io.Raw, events: np.ndarray, event_id: list[int], event_timestamp: np.ndarray, baseline: tuple[float, float]
) -> list[mne.Epochs]:
    """
        ...
        if event[2] in event_id:
            ...
            for event2 in event_timestamp:
                if event2[0] == event_id_unique:
                    ...
                    lie_deception_event_duration.append(tmax)

    return epochs_list, lie_deception_event_duration
